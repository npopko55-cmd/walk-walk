...
[Trafic source]
   ...
   ├── MAX-канал ─→ max.ru/id222391942312_bot (одна общая ссылка, считается как Direct в MAX мини-аппе)
...
**2. "Источник × мессенджер" (новая, для платных каналов) — 25 целей:**
...
**Почему нет "Посевы MAX":** в MAX весь трафик идёт через единый канал → бот, и разделить «канал A vs канал B» внутри MAX невозможно (одна ссылка для всех). Существующая воронка "MAX Mini App" уже всё покрывает.
...
**Новые 4** (нужно настроить руками в кабинете):
